...
# SKIP_BUILD=1 keeps whatever is already in build/. The shipping binary is a CI
# artifact (only an SDK 26 build turns on Liquid Glass), so the release flow is:
# assemble and sign the bundle from that artifact, then run this with SKIP_BUILD=1
# so the local SDK cannot overwrite it with a compat-look binary.
[[ "${SKIP_BUILD:-0}" == 1 ]] || "$REPO/scripts/build.sh"
