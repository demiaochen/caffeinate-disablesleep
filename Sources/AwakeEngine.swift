...
final class AwakeEngine: ObservableObject {
    ...
    /// Bumped on every optimistic flip, so a system read that started earlier
    /// knows it is stale by the time it resolves.
    ...
    func popoverAppeared() {
        ...
        // released in a terminal shows up immediately, and notice the rule file
        // coming or going the same way.
        Task {
            await syncLidLockFromSystem()
            await reconcilePasswordless()
        }
    ...
    ///
    /// The first engage explains itself first (what the rule is, what a closed
    /// lid costs). The switch stays put until that is accepted: nothing has been
    /// promised yet, so there is nothing to revert.
    func setLidLock(_ on: Bool) {
        guard lidLockEngaged != on else { return }
        guard on, LidNotice.needsExplainer else {
            applyLidLock(on)
            return
        }
        // Deferred a turn so the tap handler returns before the modal opens.
        Task { @MainActor in
            if LidNotice.showExplainer() { applyLidLock(on) }
        }
    }

    private func applyLidLock(_ on: Bool) {
        guard lidLockEngaged != on else { return }
        lidLockEngaged = on
        pendingLidTarget = on
        lidGeneration &+= 1
    ...
    private func runLidWorkerIfIdle() {
        ...
        lidLockTask = Task {
            while let target = pendingLidTarget {
                ...
                if !ok {
                    // Re-check after the read, not before it: a tap landing
                    // during `isEngaged` would otherwise be overwritten by this
                    // now stale value and never re-applied to the UI.
                    let real = await LidLock.isEngaged()
                    if pendingLidTarget == nil { lidLockEngaged = real }
                }
            }
    ...
    func syncLidLockFromSystem() async {
        ...
        // The generation check covers a toggle that both starts and finishes
        // while the read above is in flight, which the two nil checks alone
        // would miss.
        guard lidLockTask == nil, pendingLidTarget == nil, generation == lidGeneration else { return }
        lidLockEngaged = engaged
    }

    /// Re-reads whether the passwordless rule is installed. Deliberately not part
    /// of `syncLidLockFromSystem`, which runs on every panel open and on every
    /// external pmset change: this probe spawns sudo, and with no rule installed
    /// each one writes a refused non-interactive attempt to the auth log. The
    /// answer only moves when this app installs or removes the rule, so it is
    /// read at launch and after each privileged call.
    func refreshPasswordless() async {
        passwordless = await LidLock.isPasswordless()
    }

    /// Installs or removes the passwordless rule from the settings window. One
    /// admin dialog either way. The switch waits for the result rather than
    /// flipping first: a password prompt is its own feedback, and removal also
    /// clears the lid setting, so the panel has to be re-read afterwards.
    func setLidPermission(_ install: Bool) {
        guard passwordless != install else { return }
        Task {
            let ok = install ? await LidLock.installRule() : await LidLock.removeRule()
    ...
    /// Catches the rule being removed (or restored) behind our back, typically a
    /// `sudo rm` in a terminal. Testing for the file spawns nothing, so this can
    /// run on every panel open, and the real probe only runs when the answer
    /// looks like it moved.
    func reconcilePasswordless() async {
        guard LidLock.ruleFileExists != passwordless else { return }
        await refreshPasswordless()
    }
    ...
    private func watchPMPreferences() {
        ...
        source.setEventHandler { [weak self] in
            ...
            if !events.isDisjoint(with: [.delete, .rename]) {
                ...
                source.cancel()
                Task { @MainActor in
                    // Re-arm first, then read. Reading first leaves a window
                    // where no watcher exists and the change that lands in it is
                    // never seen.
    ...
    func prepareForTermination() {
        endTask?.cancel()
        releaseAssertions()
        // A password dialog still on screen belongs to a child process that
        // would outlive us. Authenticating it after we are gone would set the
        // flag with no app left to clear it.
        LidLock.cancelPendingPrompt()
