...
/// The version, which opens the window holding the launch settings and the
/// source link. Same weight as it had as plain text: this is a way in, not a
/// call to action.
struct VersionButton: View {
    ...
    var body: some View {
        ...
        .help("Settings and source (⌘,)")
        .accessibilityLabel("Settings and source")
    }
}
...
struct LidLockRow: View {
    ...
    /// Three cases, all true statements, each saying only what the user needs
    /// next. Once toggling is silent there is nothing to warn about, so the
    /// caption drops the password talk entirely and describes what quitting
    /// does. Accounts that cannot take the rule (not an admin, or a username the
    /// sudoers line cannot carry) are asked every time, and quit cannot clear
    /// the setting silently for them either.
    private var caption: String {
        if engine.passwordless { return "Turns off when you quit the app" }
        if LidLock.canInstallRule { return "Needs your password the first time" }
        return "Needs your password every time"
    }
}
