...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        Task { @MainActor in
            // Warm the admin-group check off the main thread: it shells out to
            // `id`, and its first reader is a SwiftUI body during the first
            // panel layout.
            await Task.detached { _ = LidLock.canInstallRule }.value
            await AwakeEngine.shared.syncLidLockFromSystem()
            await AwakeEngine.shared.refreshPasswordless()
...
enum StatusIcon {
    /// Main thread only: both call sites are the app delegate and a Combine sink
    /// already hopped to `DispatchQueue.main`. Spelled unsafe because nothing in
...
enum LoginItem {
    /// Last known registration state. SwiftUI re-evaluates a `@State` default on
    /// every body pass, and the panel's body re-runs once a second while a timed
    /// session's countdown ticks, so the row reads this cache instead of asking
    /// ServiceManagement over and over. `refresh()` runs at launch and on every
    /// panel open, which is the only time System Settings can have changed it.
    private(set) nonisolated(unsafe) static var isEnabled = false
